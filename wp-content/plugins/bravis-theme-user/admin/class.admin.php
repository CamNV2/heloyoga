...
if (! class_exists ( 'Bravis_Theme_User_admin' )) {

	class Bravis_Theme_User_admin {  
		...
		function __construct() {
			...
			add_filter( 'plugin_action_links_' . bravisthemeuser()->basename, array( $this, 'plugin_action_links' ) );
		}
		...
		function add_admin_page_main() {
			...
			?>
			<h1><?php esc_html_e('Bravis Theme User', 'ct-user-form'); ?></h1>
		...
		function add_admin_tab_content() {
			...
			$tab = apply_filters('ct-user-form/inc/admin/tab/template', bravisthemeuser()->plugin_dir . "admin/html_tab_$current_tab.php");
