...
class BRA_Breadcrumb
{
    ...
    function __construct( $args = array() )
    {
        ...
        if($page_on_front) {
            ...
            $home_label = esc_html__( 'Home', 'wellco' );
        }
        $args = wp_parse_args( $args, array(
            'home_label'            => $home_label,
            '404_label'             => esc_html__( 'Not Found', 'wellco' ),
            'search_results_label'  => esc_html__( 'Search Results', 'wellco' ),
    ...
    function add_tag_entry()
    {
        ...
        $this->add_entry(
            esc_html__( 'Tag:', 'wellco' ).' '.single_tag_title( '', false ) ).' '.get_tag_link( $queried_object->term_id
        );
    }
    ...
    function add_author_entry()
    {
        ...
        $this->add_entry(
            esc_html__( 'Author:', 'wellco' ).' '.$userdata->display_name
        );
    }
    ...
    function add_tax_entry()
    {
         ...
         */
        $entries = apply_filters( 'wellco_breadcrumb_taxonomy', $entries, $this_term );
    ...
    function add_paged_entry()
    {
        ...
        {
            $this->add_entry(
                apply_filters( 'wellco_breadcrumb_paged', sprintf( 'Page %s', $page ) ),
                ''
            );
        }
    }
    ...
    function add_single_ancestor_entry( $post )
    {
         ...
         */
        $entries = apply_filters( 'wellco_breadcrumb_single', $entries, $post );
